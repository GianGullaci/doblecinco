			...
			<div class="row-fluid sortable">		
				<div class="box span12">
					<div class="box-header" data-original-title>
						<h2><i class="halflings-icon user"></i><span class="break"></span>Notas Home</h2>
						<div class="box-icon">
						...
						<table class="table table-striped table-bordered bootstrap-datatable " id="tabla-notas">
						  ...
						  <tbody>
							...
							<?php
								...
								order by home desc, orden_home asc" ;
								// echo $query;
								...
								while($row = mysqli_fetch_array($result)) {
									...
									if ($row['home']==1){
										//echo '<td class="remove-home"><span style="display:none;">0'.$row['orden_home'].'</span><a class="btn btn-success"><i class="halflings-icon white ok"></i></a></td>';
										echo '<td class="remove-home"><span style="display:none;">'.$i.'</span><a class="btn btn-success"><i class="halflings-icon white ok"></i></a></td>';
										$i.="0";
									}
									else{
										echo '<td class="add-home"><span style="display:none;">a</span><a class="btn btn-danger  "><i class="halflings-icon white remove"></i></a></td>';
									}
...
// 		$('a.btn-danger').click(function(e) {
		...
		$("#tabla-notas tbody").sortable({
			...
			start:function(event, ui){
			 ...
			 update: function(event, ui) {
				...
				$.ajax({
						type: 'get',
						url: 'includes/reordenar-home.php',
						data: 'ajax=1&nota=' + id_nota + '&orden_home=' + orden,
						success: function(data) {
							var res = $.parseJSON(data);
							if (res.modificado==true){
							    if (res.home==0){
								  //tengo que convertirlo en una cruz
 								  ...
 								  td.removeClass("remove-home");
 								  td.addClass("add-home");
 								  $("#record-"+id_nota+" td.add-home a").removeClass("btn-success");
 								  $("#record-"+id_nota+" td.add-home a").addClass("btn-danger");
 								  $("#record-"+id_nota+" td.add-home a i").removeClass("ok");
 								  $("#record-"+id_nota+" td.add-home a i").addClass("remove");
							    }
							    else if (res.home==1){
								  //tengo que convertirlo en un tilde

 								  td.removeClass("add-home");
 								  td.addClass("remove-home");
								  $("#record-"+id_nota+" td.remove-home a").removeClass("btn-danger");
 								  $("#record-"+id_nota+" td.remove-home a").addClass("btn-success");
 								  $("#record-"+id_nota+" td.remove-home a i").removeClass("remove");
 								  $("#record-"+id_nota+" td.remove-home a i").addClass("ok");
		...
		$('#tabla-notas tbody').on( 'click', 'td.remove-home', function (e) {
				e.preventDefault();
			...
			$('#tabla-notas tbody').on( 'click', 'td.add-home', function (e) {
				e.preventDefault();
				...
				var cell = table.cell( this );
 				...
 				$.ajax({
 					type: 'get',
 					url: 'includes/add-home.php',
 					data: 'ajax=1&id_nota=' + id_nota,
 					success: function(data) {
 						var res = $.parseJSON(data);
 						if (res.modificado==true){
							cell.data('<span style="display:none;">0'+res.orden_home+'</span><a class="btn btn-success"><i class="halflings-icon white ok"></i></a>').draw();
							td.removeClass("add-home");
							td.addClass("remove-home");
