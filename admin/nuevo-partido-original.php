<?php
	if (isset($_POST['torneo'])){
		...
		$club_local = $_POST['local'];
		$club_visitante = $_POST['visitante'];
		$categoria = $_POST['categoria'];
		
		//como los equipos estan definidos por club/categoria los buscamos.
		
		//equipo local
		$query = "SELECT * FROM equipos 
			  where clubes_id_club=".$club_local." 
			  AND categorias_id_categoria = ".$categoria ;
		$result = $mysqli->query($query); 
		if($row = mysqli_fetch_array($result)) {
			$id_equipo_local= $row['id_equipo'];
			
		}
		else{
		  die("Equipo local inexistente");
		}
		
		//equipo visitante
		$query = "SELECT * FROM equipos 
			  where clubes_id_club=".$club_visitante." 
			  AND categorias_id_categoria = ".$categoria ;
		$result = $mysqli->query($query); 
		if($row = mysqli_fetch_array($result)) {
			$id_equipo_visitante= $row['id_equipo'];
		}
		else{
		  die("Equipo visitante inexistente");
		}
...
<body>
	...
	<script>
		...
		$(document).ready(function() {
			$.getJSON("includes/select_lugar.php",{club1: $('#local').val(),club2: $('#visitante').val(), ajax: 'true'}, function(j){
			...
			$("select#local").change(function(){
				$.getJSON("includes/select_lugar.php",{club1: $(this).val(),club2: $('#visitante').val(), ajax: 'true'}, function(j){
					var options = '<option value="0">A definir</option>';
					for (var i = 0; i < j.length; i++) {
						options += '<option value="' + j[i].id + '">' + j[i].direccion + '</option>';
					}
			...
			$("select#visitante").change(function(){
				$.getJSON("includes/select_lugar.php",{club2: $(this).val(),club1: $('#local').val(), ajax: 'true'}, function(j){
					var options = '<option value="0">A definir</option>';
					for (var i = 0; i < j.length; i++) {
						options += '<option value="' + j[i].id + '">' + j[i].direccion + '</option>';
					}
