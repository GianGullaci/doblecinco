							...
							<?php
								...
								while($row = mysqli_fetch_array($result)) {
									...
									echo '<td class="center">
										      ...
										      <a class="btn btn-info" href="editar-publicidad.php?id='.$row['id_publicidad'].'">
...
// 		$('a.btn-danger').click(function(e) {
		...
		$( '#tabla-publicidades' ).on( 'click', 'a.btn-warning', function(e) {
			...
			if (confirm("Esta seguro que desea archivar el registro?")){
				...
				$.ajax({
					type: 'get',
					url: 'includes/archivar-publicidad.php',
					data: 'ajax=1&id=' + parent.attr('id').replace('record-',''),
					success: function(data) {
						var res = $.parseJSON(data);
						if (res.deleted==true){
						  ...
						  alert("El registro no puede ser archivado "+res.razon);
						}
					}
				});
			}
