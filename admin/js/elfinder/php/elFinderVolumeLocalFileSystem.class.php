...
class elFinderVolumeLocalFileSystem extends elFinderVolumeDriver {
	...
	protected function _mkdir($path, $name) {
		...
		if (@mkdir($path)) {
			...
			//
			$sql = 'INSERT INTO galerias (ruta_galeria, nombre_galeria) VALUES ("'.$path.'", "'.$name.'")';
			//$sql = sprintf($sql, $this->tbf, $path,  $mime);
			 //echo $sql;
			$this->query($sql);
	...
	protected function _copy($source, $targetDir, $name) {
		$sql = "INSERT INTO galerias (ruta_galeria,nombre_galeria) ('".$targetDir.DIRECTORY_SEPARATOR.$name."', '".$name."')";
		$this->query($sql);
	...
	protected function _move($source, $targetDir, $name) {
		...
		$sql = "UPDATE galerias SET ruta_galeria='".$target."', nombre_galeria='".$name."' WHERE ruta_galeria='".$source."'";
		// echo $sql;
		$this->query($sql);
		$sql2 = "UPDATE galerias SET ruta_galeria = REPLACE(ruta_galeria, '".$source."', '".$target."') WHERE ruta_galeria LIKE ('".$source."/%')";
		// echo $sql2;
		$this->query($sql2);
	...
	protected function _rmdir($path) {
		$sql = "DELETE FROM galerias WHERE ruta_galeria='".$path."'";
		$this->query($sql);
		return @rmdir($path);
	}
