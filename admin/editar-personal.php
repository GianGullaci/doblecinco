...
<?php
	...
	if (isset($_POST['nombre'])){
		...
		if(isset($_FILES["foto"]) && !empty($_FILES["foto"]["name"])){
			...
			foreach (glob($target_dir . 'personal.' . '*') as $filename) {
			...
			if ($uploadOk == 0) {
				$msj = $msj."<br>El archivo de la foto del personal no ha sido cargado";
	...
	$query = "SELECT * FROM personal where id_personal=".$_GET['id'] or die("Error in the consult.." . mysqli_error($mysqli));
	$result = $mysqli->query($query); 
	if ($row = mysqli_fetch_array($result)) {
		$id_personal = $row['id_personal'];
		$nombre_personal = ucfirst($row['nombre']);
		$fecha_nacimiento ="";
			...
			<div class="row-fluid sortable">
				<div class="box span12">
					<div class="box-header" data-original-title>
						<h2><i class="halflings-icon edit"></i><span class="break"></span>Editar Personal</h2>
						<div class="box-icon">
					...
					<div class="box-content">
						<form class="form-horizontal" action="?id=<?=$id_personal?>" method="post" enctype="multipart/form-data">
						  <fieldset>
							<div class="control-group">
								<label class="control-label" for="nombre">Nombre</label>
								<div class="controls">
									<input class="input-xlarge focused" required id="nombre" name="nombre" type="text" value="<?=$nombre_personal?>">
								</div>
							  </div>
							<div class="control-group">
							...
							<div class="control-group">
								...
								<div class="controls">
								<?php
								  if (file_exists("../img/personal/".$id_personal."/".$foto)){
								?>
								  <img class="grayscale" src="../img/personal/<?=$id_personal?>/<?=$foto?>" alt="Foto" width="150" style="margin-left: 20px">
